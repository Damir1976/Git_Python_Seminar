...
# def isCyrillic(text: k): 	
#    return bool(isCyrillic('[а-яА-Я]', text)) 
import re 
def cyrillic(text):
...
text = k.upper() 
if cyrillic(text):	
    print(sum([k for i in text for k, v in point_ru.items() if i in v])) 
else: 	
    print(sum([k for i in text for k, v in point_en.items() if i in v]))  
